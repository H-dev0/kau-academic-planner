...
async function saveProgress() {
  ...
  if (!state.user) {
    saveStatus.textContent = "سجل الدخول لحفظ التقدم.";
    return;
  }
  ...
  try {
    ...
    saveStatus.textContent = "تم حفظ التقدم.";
  } catch {
    saveStatus.textContent = "تعذر حفظ التقدم.";
  }
}
...
function render() {
  ...
  plannerTitle.textContent = isCatalogOnly() ? titleName + " - فهرس البرنامج" : titleName + " - مخطط المقررات";
  ...
  majorStatus.textContent = isCatalogOnly()
    ? "فهرس فقط - " + (state.program.catalog_note || "الخطة الدراسية مطلوبة")
    : String((state.program.courses || []).length) + " مقرر محمل";

  renderChecklist();
  renderList(
    completedList,
    sortByLevelThenCode(plan.completed),
    "لم يتم تحديد مقررات مجتازة بعد.",
    "completed",
  );
  renderList(
    recommendedList,
    recommended,
    "حدد المقررات المجتازة لعرض المقترحات.",
    "recommended",
  );
  renderList(availableList, plan.available, "لا توجد مقررات متاحة.", "available");
  renderList(blockedList, plan.blocked, "لا توجد مقررات محجوبة.", "blocked");
}
...
loginForm.addEventListener("submit", async (event) => {
  ...
  if (!/^\d{7}$/.test(studentId)) {
    saveStatus.textContent = "يجب أن يتكون الرقم الجامعي من 7 أرقام فقط.";
    usernameInput.focus();
    return;
  }
  saveStatus.textContent = "جاري تسجيل الدخول...";
  try {
    ...
    await loadProgress();
    render();
    saveStatus.textContent = "تم تسجيل الدخول. سيتم حفظ التقدم.";
  } catch {
    saveStatus.textContent = "فشل الدخول. الرقم الجامعي يجب أن يكون 7 أرقام فقط.";
  }
});
...
logoutButton.addEventListener("click", async () => {
  ...
  state.user = null;
  render();
  saveStatus.textContent = "تم تسجيل الخروج.";
});
